...
import { GroupUserInfoType, MessageItemType, MessageSendType } from "@/interface/MessageSendType";
...
`
  ...
  function extractMessageContent(item: MessageItemType) {
    if (item.type == 'text') {
      ...
      item?.data?.content.forEach((t) => {
        t.message.forEach((tt) => extractMessageContent(tt))
      })
    }
  }
  const body = getAIMessageInstance()
...
export async function concludePersonByAI(person: GroupUserInfoType, groupMessages: Message[]) {
  ...
  personMessages.forEach(t => {
    t.message.forEach((item) => {
      extractMessageContent(item)
    })
  })
  // const allMsg = []
  // groupMessages.forEach(item => {
  //   item.message.forEach((t) => {
  //     allMsg.push(`${item.user_id}-${JSON.stringify(t)}`)
  //   })
  // })
  // userMsg.content.push({
  //    type: 'text', 
  //    text: `
  //    以下内容为聊天的总内容，其中消息用换行分割，并且消息开头为QQ号，你只需关注目标的QQ，其他的是为其他人的发言
  //   ${allMsg.join('\n')}
  //   ` 
  // })
