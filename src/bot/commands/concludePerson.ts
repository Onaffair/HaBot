...
export default createCommand({
  ...
  handle: async (session) => {
    ...
    const person = group.listen.find(item => item.group_id == session.groupId)
    ?.members[nameIndex] as GroupUserInfoType
    if (!person) return
    
    logger.info(`正在查询 ${person.card} 的成分`)
    const messages = await getGroupMessage(session.groupId, 200).then(res => res?.data?.messages || []).catch(() => [])

    let res = await concludePersonByAI(person, messages) as string
    if (!res?.trim()) {
