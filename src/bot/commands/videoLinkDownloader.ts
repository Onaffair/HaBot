import { downloadFromUrl, getVideoMetaInfo, getVideoUrl } from "@/api/common";
...
const videoDownloaderCmd: Command = {
  ...
  match: (session) => {
    ...
    ].some(t => session.textContent.includes(t))
  },
  handle: async (session) => {


    let text = session.textContent

    if (text.includes('https://b23.tv/')) {
      const redReg = /https?:\/\/b23\.tv\/[A-Za-z0-9]+/
      const match = session.textContent.match(redReg)
    ...
    const videoRes = await getVideoUrl({ bvid, cid, fnval: 4048, qn: 80, fourk: 1 }, { headers })
    const { data } = videoRes
    const { dash } = data
    const { video, audio } = dash
    const [{ baseUrl: videoUrl }] = video
    const [{ baseUrl: audioUrl }] = audio
    // logger.info(videoUrl, audioUrl)

    const [videoBuffer, audioBuffer] = await Promise.all(
      [
        downloadFromUrl(videoUrl, { responseType: 'arraybuffer', headers }) as Promise<Buffer>,
        downloadFromUrl(audioUrl, { responseType: 'arraybuffer', headers }) as Promise<Buffer>
      ]
    )
